...
def old_get_profiles(phi10,phi20,lam1=1,lam2=1,lam12=1,m1=1e-10,m2=1e-10,fr=1e17,rstart=10.0,dx=1.0,display=False,iteration=100):
    N = 8
    ...
    while True:
        ...
        if len(mask1) > 0 or len(mask2) > 0 :
            ...
            if dphi1[-1] > 1.0e-5*dphi1[0] and dphi2[-1] > 1.0e-5*dphi2[0] :
                break
        r1 = abs(dphi1[-1]/phi1[-1]),abs(dphi2[-1]/phi1[-1])
        r2 = tuple(1/r for r in r1)
...
def get_profiles(phi10,phi20,lam1=1,lam2=1,lam12=1,m1=1e-10,m2=1e-10,fr=1e17,rstart=10.0,dx=1.0,display=False,iteration=100):
    ...
    while True:
        ...
        if len(mask1) > 0 or len(mask2) > 0 :
            ...
            if M > 1000: M = 1000
            x,y = interp(x,y,N,M)
            rat1 = abs(dphi1[-1]/phi1[-1]),abs(dphi2[-1]/phi1[-1])
            rat2 = tuple(1/r for r in rat1)
            rat = max(max(rat1),max(rat2))
            #print(rat)
            if rat < 5:
                break
    x,y = interp(x,y,N,10000)
...
def scan(phis,cores=1,parallel={"_print":True,"split":True},**kwargs):
    ...
    res = parallel(f,cores=cores,**parallel)(phis,**kwargs)
